...
const ChildMenu = () => {
    const integrations = useMemo(() => {
        try {
            return localStorage.getItem("child-integrations")
                ? JSON.parse(localStorage.getItem("child-integrations"))
                : [];
        } catch (err) {
            return [];
        }
    ...
    const isSubMenuActive = useMemo(() => {
        return (
            location.pathname.includes("/child/indiamart-data") ||
            location.pathname.includes("/child/fb-data") ||
            location.pathname.includes("/child/tradeindia-data")
        );
    }, [location.pathname]);
    ...
    return (
        <div className="main_menu f-center">
            <ul className="md:flex md:items-center md:gap-3 whitespace-nowrap">
                ...
                <li>
                    <MenuLink to="/dev/child/search-history">
                ...
                {/*<li>
                    <MenuLink to="/admin/uploaded-data">
                            ...
                            <div className="absolute top-full left-0 w-full hidden group-hover:block z-10">
                                <ul className="mt-2 space-y-0 bg-white border border-gray-200 rounded-md overflow-hidden shadow-md">
                                    {integrations.includes(1) && (
                                        <li>
                                            <SubMenuLink to="/dev/child/indiamart-data">IndiaMART Data</SubMenuLink>
                                        </li>
                                    )}
                                    {integrations.includes(2) && (
                                        <li>
                                            <SubMenuLink to="/dev/child/fb-data">Facebook Data</SubMenuLink>
                                        </li>
                                    )}
                                    {integrations.includes(3) && (
                                        <li>
                                            <SubMenuLink to="/dev/child/tradeindia-data">TradeIndia Data</SubMenuLink>
                                        </li>
                                    )}
                                </ul>
                            </div>
